...
const counts = useMemo(() => ({
  ...
  q1: tasks.filter(t => t.status === 'active' && t.isUrgent && t.isImportant).length,
  q2: tasks.filter(t => t.status === 'active' && !t.isUrgent && t.isImportant).length,
  q3: tasks.filter(t => t.status === 'active' && t.isUrgent && !t.isImportant).length,
  q4: tasks.filter(t => t.status === 'active' && !t.isUrgent && !t.isImportant).length,
}), [tasks]);
```
...
**Example Styles**:
```tsx
<div className="inline-flex items-center gap-1.5 rounded-full bg-[rgb(var(--color-surface))] 
